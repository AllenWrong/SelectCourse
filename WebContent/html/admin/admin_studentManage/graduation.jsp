...
<div class="layui-tab layui-tab-card">
    <div class="layui-tab-content">
        <form action="" class="layui-form" method="post" id="search">
            <div class="layui-form-item">
                ...
                <div class="layui-inline">
                    ...
                    <div class="layui-inline">
                        <input class="layui-input date" type="text" id="start"  name="start" placeholder="开始时间">
                    </div>
                </div>
                ...
                <div class="layui-inline">
                    <div class="layui-input-block">
                        <button type="button" class="layui-btn" lay-submit="" id="dosearch" lay-filter="dosearch">查询</button>
                    </div>
                </div>
            </div>
        </form>
        ...
        <!--表格上方按钮开始-->
        <div  style="display: none" id="graduation_ToolBar" lay-filter="graduation_ToolBar">
        ...
        <!--动态表格开始-->
        <table id="graduation_table" lay-filter="graduation_table"> </table>
        <!--动态表格结束-->

        <!-- 添加的弹出层开始 -->
        <div style="display: none;padding: 20px;margin-top: 30px" id="addDiv">
        <form class="layui-form " action="" lay-filter="addGraduation" id="addGraduation">
            <div class="layui-form-item">
                <div class="layui-inline">
                    <label class="layui-form-label">学号:</label>
                    <div class="layui-input-inline">
            ...
            <div class="layui-form-item">
                <div class="layui-inline">
                    <label class="layui-form-label">变动日期:</label>
                    <div class="layui-input-inline">
                        <input type="text" name="changeDate"  lay-verify="required" autocomplete="off"
                               class="layui-input changeDate">
                    </div>
                </div>
                <div class="layui-inline">
                    <label class="layui-form-label">变动信息:</label>
                    <div class="layui-input-inline">
                        <input type="text" name="changeMessage" lay-verify="required" autocomplete="off"
                               class="layui-input" >
                    </div>
                </div>
            </div>
        ...
        <div style="display: none;padding: 20px;margin-top: 30px" id="updateDiv">
            <form class="layui-form " action="" lay-filter="updateGraduation" id="updateGraduation">
                <div class="layui-form-item">
                    <div class="layui-inline">
                        <label class="layui-form-label">学号:</label>
                        <div class="layui-input-inline">
...
<script>
    layui.use(['element','form','table', 'layer', 'laypage','laydate'],function(){
        ...
        var tableIns = table.render({
            id:'graduation'
            ,elem: '#graduation_table'
            , url: '../../../json/graduation.json' //请求路径
            ,defaultToolbar:[]
            , toolbar: '#graduation_ToolBar'
            , cellMinWidth: 80 //全局定义常规单元格的最小宽度，layui 2.2.1 新增
            , cols: [
                [
                    { type: 'checkbox',fixe: 'left'}
                    , {field: 'studentNum', title: '学号',align:'center',width:250}
                    , {field: 'studentName', title: '学生姓名',align:'center',width:250}
                    , {field: 'changeDate', title: '变动日期',align:'center',width:250}
                    , {field: 'changeMessage', title: '变动信息',align:'center'}
                ]
            ]
        });
        ...
        function fuzzy_search(){
            var time = $('#start');//开始时间
            var studentNum = $('#studentNum');//学生姓名

            console.log(time.val());
            console.log(studentNum.val());
            //执行重载 （发送了两次请求。上面table.render 向后台发送了一次请求，这里table.reload还会向后台放松一次请求，两次请求路径相同）
            table.reload('graduation', {
                page: {
                    curr: 1 //重新从第 1 页开始
                },
                where: {
        ...
        //监听头部
        table.on('toolbar(graduation_table)',function (obj) {
            var checkStatus = table.checkStatus('graduation'); //获取选中的行 table.render里的id对应的值
        ...
        function openUpdateCourse(data){
            updateIndex=layer.open({
                ...
                area:['800px','300px'],
                success:function(index){
                    form.val("updateGraduation",data);
        ...
        form.on("submit(updateSubmit)",function(obj){
            ...
            var jsonObj1 = {
                ...
                'changeDate': params1.changeDate,
                'changeMessage': params1.changeMessage
            };
        ...
        function batchDelet(data) {
            ...
            if (data.length > 0) {
                for (var i in data) {
                	...
                	date.push(data[i].changeDate);
                }
                console.log(stuNum);
                console.log(date);
                layer.confirm('确定删除选中的数据？', {icon: 3, title: '提示信息'}, function (index) {
                    $.post('url',{'stuNum':stuNum,'date':date},function (data) { //url为请求路径
                        tableIns.reload();
                        layer.close(index);
